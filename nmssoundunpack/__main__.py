...
def main(source: Path, destination: Path, tmp: Path, keep: bool):
    for psarc in psarcs:
...
if __name__ == "__main__":
    ...
    source_path = Path.cwd() / Path(args.source)
    destination_path = Path.cwd() / Path(args.destination)
    temporary_path = Path.cwd() / Path(args.tmp) if args.tmp is not None else destination_path / Path(__file__).parent.name
    main(source_path, destination_path, temporary_path, args.keep)
